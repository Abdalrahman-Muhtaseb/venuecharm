...
│   ├── layout/      # PublicNavbar (logo + hamburger; SearchRow compact on /venues; NotificationBell; host↔guest switch), HostSidebar (desktop: fixed full-height column; bottom: HostProfileLink with avatar+name+signout; mobile: HostMobileNav Sheet drawer; "Exit hosting" above HostProfileLink), HostHeaderBar (shared base: pre-rendered icon ReactNode + optional action + notification bell + theme toggle), HostPanelHeaderBar (pathname→title+icon lookup, used in (panel) layout), NotificationBell (Realtime dropdown, unread badge, mark-read, deep-links), NotificationsPanel (full /notifications page, reused by /host/notifications), Footer, AuthShell, BrandBackground (subtle themed gradient/blobs)
│   ├── admin/       # AdminActionButtons (approve/suspend), AdminSubNav, UserRoleButton,
│   │                # AdminCancelBookingButton, SeedDataPanel, DangerZonePanel, MonthlyBarChart (dependency-free CSS bars for the Analytics tab), BackfillPaymentsPanel (admin tool to backfill stripe_transfer_id/stripe_refund_id from Stripe API)
