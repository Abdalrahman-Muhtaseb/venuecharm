...
import {
  ...
  Timer, ExternalLink, Banknote, BookOpen, ShieldAlert,
  CheckCircle2, XCircle, CalendarCheck2, ChevronRight,
  Pencil, Send, Trash2,
} from 'lucide-react'
...
export default async function HostListingDetailPage({ params }: { params: { id: string } }) {
  ...
  const [
    ...
    supabase
      .from('venues')
      .select(
        `id, host_id, title, description, address, city, capacity,
      ...
      .eq('host_id', user.id)
      .single(),
    supabase.from('bookings').select('*', { count: 'exact', head: true }).eq('venue_id', params.id),
    supabase
      .from('bookings')
    ...
    supabase.from('reviews').select('rating').eq('venue_id', params.id),
  ...
  return (
    <div className="space-y-6 pb-12">
      ...
      <nav className="flex items-center gap-1.5 text-sm text-muted-foreground">
        <Link href="/host/listings" className="flex items-center gap-1 transition-colors hover:text-foreground">
          <ArrowLeft className="h-3.5 w-3.5" />
          {isHe ? 'הנכסים שלי' : 'My listings'}
        </Link>
        <ChevronRight className="h-3.5 w-3.5 shrink-0" />
        <span className="truncate font-medium text-foreground">{venue.title}</span>
      </nav>
      ...
      <div className="space-y-4">
        <div className="flex flex-wrap items-start justify-between gap-3">
          ...
          <Badge
            variant="outline"
            className={cn('shrink-0 px-3 py-1 text-xs font-semibold', statusCfg.className)}
          >
            {statusCfg.label}
          </Badge>
        </div>
        ...
        <div className="flex flex-wrap items-center gap-2 rounded-xl border bg-muted/30 px-4 py-3">
          <Button asChild variant="default" size="sm">
            <Link href={`/host/listings/${venue.id}/edit`}>
              <Pencil className="me-1.5 h-4 w-4" />
              {isHe ? 'עריכה' : 'Edit listing'}
            </Link>
          </Button>

          {venue.status === 'DRAFT' ? (
            <RequestApprovalButton venueId={venue.id} locale={locale} />
          ) : (
            <DeleteVenueButton venueId={venue.id} locale={locale} />
          )}
          ...
          {venue.status === 'ACTIVE' && (
            <Button asChild variant="outline" size="sm" className="ms-auto">
              <Link href={`/venues/${venue.id}`} target="_blank">
                ...
                {isHe ? 'צפייה ציבורית' : 'View public page'}
              </Link>
            </Button>
          )}
        </div>
