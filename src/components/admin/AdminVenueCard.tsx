...
import { Building2, MapPin, Star, CheckCircle2, Banknote, BarChart2 } from 'lucide-react'
...
export interface AdminVenueCardRow {
  id: string
  title: string
  city: string
  status: string
...
export function AdminVenueCard({
  ...
  rating: { avg_rating: number; review_count: number } | undefined
}) {
  ...
  return (
    <div className="overflow-hidden rounded-xl border bg-background">
      {/* Photo */}
      <Link href={`/admin/${v.id}`} className="block">
      ...
      <div className="flex flex-col gap-3 p-4">
        ...
        <div className="flex items-center gap-1.5 text-xs">
          <span className="shrink-0 text-muted-foreground">{isHe ? 'מארח' : 'Host'}</span>
          <Link
            href="/admin/users"
            className="truncate font-medium text-primary/80 underline-offset-4 hover:text-primary hover:underline"
          >
            {hostName}
          </Link>
        </div>
